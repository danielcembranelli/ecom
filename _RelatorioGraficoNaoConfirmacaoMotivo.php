		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: 'Propostas Nao Confirmadas - Motivo'
					},
					plotOptions: {
						pie: {
...
$Sql = mysql_query("SELECT mp.labelMp, count(*) as total FROM proposta p inner join motivo_proposta mp on (mp.idMp=p.idMp) ".$montaWhere." group by mp.labelMp order by total") or die (mysql_error());
while ($dom = mysql_fetch_array($Sql)){
	?>
							['<?=$dom[labelMp]?>',<?=$dom[total]?>],
		<? } ?>					
						]
					}]
				});
