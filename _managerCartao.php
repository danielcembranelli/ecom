...
if (r==true)
  {
	window.location.replace("index.php?_m=livesupport&_a=managerCartao&step=3&id="+valor)
  }
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<td width="100%"><span class="smalltext">&nbsp;Manager Ação Promocional</span></td>
					</tr>
					<tr height="4">
...
if($_POST[CODIGOACAO]!=''){
	$Sql = mysql_query("Insert into clientes_cartao (Cli_ID,dtentregaCc,idVendedor,nrCc) values ('".$id."','".dataSql($_POST[DATAENTREGA])."','".$_POST[idVendedor]."',,'".$_POST[CODIGOACAO]."')");
}
if($sqlIncluir){
?>
...
<input type="hidden" name="_a" value="managerCartao"/>
...
$Sql = "SELECT l.nome as VENDEDOR, DATE_FORMAT(C.dtentregaCc,'%d/%m/%Y') as DTENTREGA, C.nrCc, A.antigoidCliente, A.Cli_ID, A.Cli_Fantasia, Cli_Nome, A.Cli_CGC, A.Cli_Contato, A.Cli_Fone1, A.Cli_DDD1, A.Cli_Endereco, A.Cli_Cidade, A.Cli_UF, A.Cli_CEP, A.dtcadCli, A.Cli_Status, A.expiraCliente, DATE_FORMAT(A.dtexpiraCliente,'%d/%m/%Y') as dtExpira, A.idVendedor FROM clientes_cartao C inner join clientes A on (A.Cli_ID=C.Cli_ID) left join login l on (l.id=C.idVendedor) where ";
...
<? } ?>
<td class="tabletitlerow" width="80" align="" nowrap>&nbsp;Cartao</td>
<td class="tabletitlerow" width="" align="" nowrap>&nbsp;Nome&nbsp;</td>
<td class="tabletitlerow" width="80" align="center" nowrap>&nbsp;Vendedor</td>
<td class="tabletitlerow" width="200" align="center" nowrap>&nbsp;Data<img src="themes/admin_default/sortasc.gif" border="0" /></td>
...
<td colspan="" align="center" valign="">
	<? if($sU[idVendedor]==$dl[id]){ $cadeado ='N';?><img src="menu/ico_locker_open.png" /><? } else { $cadeado ='S';?><img src="menu/ico_locker_close.png" /><? } ?></td>
<? } ?>
<td colspan="" width="" align="center" valign=""><?=$sU[nrCc]?></td>
...
<td colspan="" width="150" align="center" valign=""><? if($cadeado=='N'){?><a href="index.php?_m=livesupport&_a=editCliente&id=<?=$sU[Cli_ID]?>" title="Edit"><? } ?><?=$sU[VENDEDOR]?><? if($cadeado=='N'){?></a><? } ?></td>
<td colspan="" width="150" align="center" valign=""><?=$sU[DTENTREGA]?></td>
<?
if($campoOculto=='1'){	
	?>
...
<? if($sU[antigoidCliente]==0 && $sU[dtcadCli]=='0000-00-00 00:00:00'){?><a href="index.php?_m=livesupport&_a=managerCartaoAntigo&idCliente=<?=$sU[Cli_ID]?>" title="Vincular Cliente"><img src="themes/admin_default/icon_export.gif" border="0"></a><? }?>
...
for($i_pg=1;$i_pg<$quant_pg;$i_pg++) { 
				...
				echo "<td class='".$css."'><a href='index.php?_m=livesupport&_a=managerCartao&pg=".$i_pg."' title='Pag. ".$i_pg." of ".$quant_pg."'>".$i_pg."</a></td>";
        }
?>
