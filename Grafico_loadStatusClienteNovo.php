...
$ob = array();
$max = 15;
$Sql = mysql_query("SELECT DATE_FORMAT(dtcadCli,'%Y-%m') as dt,DATE_FORMAT(dtcadCli,'%m/%Y') as label, count(*) as total FROM clientes c where DATE_FORMAT(dtcadCli,'%Y-%m')!='0000-00' group by dt order by dt desc Limit 12;");
...
$Sql = mysql_query("SELECT DATE_FORMAT(dtcadProposta,'%Y-%m') as dt, count(*) as total FROM proposta group by dt desc Limit 12");
...
$Sql = mysql_query("SELECT DATE_FORMAT(dtcadProposta,'%Y-%m') as dt, DATE_FORMAT(dtcadProposta,'%m/%Y') as label FROM proposta group by dt desc Limit 12");
...
$g = new graph();
$g->title( 'Movimento dos Ultimos 12 Meses', '{font-size: 14px;}' );
$g->bg_colour = '#FFFFFF';
