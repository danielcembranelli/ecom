...
$sqlProposta = mysql_query("SELECT c.Cli_Fantasia, c.Cli_Contato, p.descricaoObra, p.tipoProposta, p.obsgeraisProposta, p.contatoProposta, pa.nome, DATE_FORMAT(p.dtcadProposta,'%d/%m/%Y') as data FROM proposta p inner join clientes c on (p.idCliente=c.Cli_ID) left join login pa on (p.idVendedor=pa.id) where p.idProposta = '".$IdProposta."'") or die ("Could not connect to database: ".mysql_error());
...
<title>Proposta N&ordm; <?=$IdProposta?></title>
		<link type="text/css" href="jquery/css/redmond/jquery-ui-1.8.2.custom.css" rel="stylesheet" />	
