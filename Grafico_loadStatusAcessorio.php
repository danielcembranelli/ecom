...
$Sql = "SELECT  count(*) as total, l.nome FROM proposta p inner join login l on (l.id=p.idVendedor) where statusProposta!='T' And DATE_FORMAT(p.dtcadProposta,'%Y-%m') = DATE_FORMAT(NOW(),'%Y-%m') group by idVendedor order by dtcadProposta desc";
...
$g->bg_colour = '#FFFFFF';
$g->title( 'Propostas Emitidas No Mes', '{font-size: 14px;}' );
