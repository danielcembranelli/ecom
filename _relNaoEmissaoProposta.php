<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<td width="100%"><span class="smalltext">&nbsp;Relatório de Cliente sem emissão de proposta</span></td>
					</tr>
					<tr height="4">
...
if($_POST[statusProposta]!=''){
	if($_SESSION[buscaUltimo][statusProposta]!=$_POST[statusProposta]){
			$_SESSION[buscaUltimo][statusProposta]=$_POST[statusProposta];
	}
}

if($_POST[statusCliente]!=''){
	if($_SESSION[buscaUltimo][statusCliente]!=$_POST[statusCliente]){
			$_SESSION[buscaUltimo][statusCliente]=$_POST[statusCliente];
	}
}
...
<input type="hidden" name="_a" value="relNaoEmissaoProposta"/>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
...
<select name="statusCliente" class="quicksearch">
<option value="TO" <? if($_SESSION[buscaUltimo][statusCliente]=='TO'){?>selected="selected"<? } ?>>Todos</option>
<option value="A" <? if($_SESSION[buscaUltimo][statusCliente]=='A'){?>selected="selected"<? } ?>>Ativo</option>
<option value="I" <? if($_SESSION[buscaUltimo][statusCliente]=='I'){?>selected="selected"<? } ?>>Inativo</option>
<option value="E" <? if($_SESSION[buscaUltimo][statusCliente]=='E'){?>selected="selected"<? } ?>>Excluido</option>
<option value="P" <? if($_SESSION[buscaUltimo][statusCliente]=='P'){?>selected="selected"<? } ?>>Prospecção</option>
</select> 

<select name="statusProposta" class="quicksearch">
<option value="TO" <? if($_SESSION[buscaUltimo][statusProposta]=='TO'){?>selected="selected"<? } ?>>Todos</option>
<option value="A" <? if($_SESSION[buscaUltimo][statusProposta]=='A'){?>selected="selected"<? } ?>>Aguardando Aprovação</option>
<option value="N" <? if($_SESSION[buscaUltimo][statusProposta]=='N'){?>selected="selected"<? } ?>>Em aberto</option>
<option value="L" <? if($_SESSION[buscaUltimo][statusProposta]=='L'){?>selected="selected"<? } ?>>Não Confirmada</option>
<option value="C" <? if($_SESSION[buscaUltimo][statusProposta]=='C'){?>selected="selected"<? } ?>>Confirmada</option>
<option value="F" <? if($_SESSION[buscaUltimo][statusProposta]=='F'){?>selected="selected"<? } ?>>Finalizada</option>
<option value="T" <? if($_SESSION[buscaUltimo][statusProposta]=='T'){?>selected="selected"<? } ?>>Excluida</option>
</select> 
...
$Sql = "SELECT c.Cli_Status, mp.labelMp, DATE_FORMAT(pr.dtcancelaProposta,'%d/%m/%Y') as dtCancela, pr.txtcancelaProposta, c.idProposta, pr.statusProposta, lo.nome as nmVProposta, c.antigoidCliente, c.Cli_ID, c.Cli_Fantasia, DATE_FORMAT(c.Cli_DataEmitido,'%d/%m/%Y') as dt, c.dtcadCli, c.idFilial, p.NOME_PATIO, c.idVendedor, l.nome FROM clientes c left join patio p on (p.ID_PATIO=c.idFilial) left join login l on (l.id=c.idVendedor) left join proposta pr on (pr.idProposta=c.idProposta) left join login lo on (lo.id=pr.idVendedor) left join motivo_proposta mp on (pr.idMp=mp.idMp)";
...
if($_SESSION[buscaUltimo][statusCliente]=='TO'){
					$montaWhere .= 'where c.Cli_Status!="E"';
				} else {
					$montaWhere .= 'where c.Cli_Status="'.$_SESSION[buscaUltimo][statusCliente].'"';
				}
...
if($_SESSION[buscaUltimo][statusProposta]=='TO'){
					$montaWhere .= 'And pr.statusProposta!="T"';
				} else {
					$montaWhere .= 'And pr.statusProposta="'.$_SESSION[buscaUltimo][statusProposta].'"';
				}
...
<td class="tabletitlerow" width="60" align="center" nowrap>&nbsp;Status</td>
...
<tr id="trid1466" class="<?=$cor?><? if($sU[statusProposta]=='L'){?> vtip<? }?>" <? if($sU[statusProposta]=='L'){?>title="<b><?=$sU[labelMp]?></b><br><?=$sU[txtcancelaProposta]?><br><i>Data: <?=$sU[dtCancela]?></i>"<? }?> onmouseover="showHighlight(this);" onmouseout="clearHighlight(this, '<?=$cor?>');">
...
<td colspan="8" width="" align="left" valign="">
...
<?
$sqlUsuario1 = mysql_query("SELECT cg.idCg, count(*) as total from contatos c left join contatos_grupo cg on (cg.idCg=c.idCg) where  idCliente = '".$sU[Cli_ID]."' And statusContato='A' group by cg.idCg");
while ($sC = mysql_fetch_array($sqlUsuario1)){
	$c[$sC[idCg]]=$sC[total];	
}
//print_r($c);
$b=0; 
$sqlUsuario2 = mysql_query("SELECT * from contatos c left join contatos_grupo cg on (cg.idCg=c.idCg) where  idCliente = '".$sU[Cli_ID]."' And statusContato='A' ORDER BY labelCg, nomeContato") or die ("Could not connect to database: ".mysql_error());
while ($sA = mysql_fetch_array($sqlUsuario2)){
$cor = ($coralternada++ %2 ? "row2" : "row1");  
?>
<tr id="trid1466" class="<?=$cor?>" onmouseover="showHighlight(this);" onmouseout="clearHighlight(this, '<?=$cor?>');">

    <td width="" align="center" valign=""><?=$sA[labelCg];?></td>
 
    <td colspan="" width="" align="center" valign=""><?=$sA[nomeContato];?></td>
    <td colspan="" align="center" <? if($sA[statusemailContato]=='E'){?>class="vtip" title="E-mail relatou erro no ultimo envio"<? } ?>><? if($sA[statusemailContato]=='E'){?><img src="themes/admin_default/icon_error.gif" /> &nbsp; <? } ?> <?=$sA[emailContato];?></td>
    <td colspan="" align="center"><?=$sA[cargoContato];?></td>
    <td colspan="" align="center">(<?=$sA[dddfone1];?>) <?=$sA[nrfone1];?></td>
    <td colspan="" align="center">(<?=$sA[dddcelular];?>) <?=$sA[nrcelular];?></td>
</tr>
<? } ?>
