...
if (r==true)
  {
	window.location.replace("index.php?_m=livesupport&_a=managerClienteAntigo&step=3&id="+valor)
  }
}
...
<? 
$Sql1 = "SELECT A.Cli_ID, A.Cli_Fantasia, Cli_Nome, A.Cli_CGC, A.Cli_Contato, A.Cli_Fone1, A.Cli_DDD1, A.Cli_Endereco, A.Cli_Cidade, A.Cli_UF, A.Cli_CEP FROM clientes A where A.Cli_ID='".$_REQUEST[idCliente]."'";

$sqlNovo = mysql_query($Sql1) or die ("Erro Cliente Atual: ".mysql_error());

$sC = mysql_fetch_array($sqlNovo)
?>
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="tborder" style="">
<tbody><tr class="row2"><td>
...
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_Fantasia'){?>selected="selected"<? } ?> value="Cli_Fantasia">Nome Fantasia:</option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_Nome'){?>selected="selected"<? } ?> value="Cli_Nome">Razão Social:</option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_Contato'){?>selected="selected"<? } ?> value="Cli_Contato">Contato:</option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_CGC'){?>selected="selected"<? } ?> value="Cli_CGC">CNPJ:</option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_Endereço'){?>selected="selected"<? } ?> value="Cli_Endereco">Endereço: </option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_Cidade'){?>selected="selected"<? } ?> value="Cli_Cidade">Cidade:</option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_UF'){?>selected="selected"<? } ?> value="Cli_UF">Estado:</option>
<option <? if($_SESSION[buscaantigo][campoForm]=='Cli_CEP'){?>selected="selected"<? } ?> value="Cli_CEP">Cep:</option>
</select><input type="text" name="quicksearch" class="quicksearch" value="<?=$_SESSION[buscaantigo][quicksearch]?>" /></td><td width="1"><img src="themes/admin_default/space.gif" width="4" height="1" /></td><td align="right" width="1"><table border="0" cellpadding="0" cellspacing="1" class="tborder"><tr><td class="navpageselected" nowrap><a onclick="javascript:document.users.submit();" href="#" title="Quick Search">Pesquisar</a></td>
