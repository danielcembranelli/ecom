...
function exclusoes($FRASE){
	$retorno=1;
	$sqlConsulta = mysql_query("SELECT emailContato FROM contatos where emailContato='".$FRASE."'") or die ("Could not connect to database: ".mysql_error());
	while($c = mysql_fetch_array($sqlConsulta)){
		if(stristr($FRASE,$c[emailContato])) {
       		$retorno=0;
		}
	}
	return $retorno;
